...
export const DECISIONS: DecisionItem[] = [
  ...
  // ============ STRATEGIC (foundational, takes longer) ============
  {
    ...
    suggestion: 'Direction is set toward Option A (wholesale swap — studio branches move to GHL). Final mapping of which 3 branches remain visible on the spiral is still being clarified — see energy-branch-content-source.',
    type: 'multi',
    progress: 75,
    status: 'partial',
    options: [
      { label: 'Option A — Full swap', description: 'Wholesale replace studio branches: studio branches exit the spiral and ship to GHL; spiral surfaces only Maddie\'s curated 3 (or her chosen mapping).', consequence: 'Cleanest content architecture. URLs/SEO impacted. Matches Maddie\'s verbatim "switch my baby branches to spiral and your kickass ones into GHL."', recommended: true },
      { label: 'Option B — Curated subset', description: 'Keep 2-3 studio branches you choose; add your GHL branches alongside; rest of studio content goes to GHL.', consequence: 'Possibly more branches on spiral total; mixed authorship; easiest revert.' },
    ...
    notes: 'Direction locked 2026-05-16 by Maddie: studio branches migrate to GHL via HTML codes. Spiral retains 3 visible (per her "inflammation, hormone health & energy/cellular health"). Final mapping still ambiguous — are the 3 visible the renamed studio branches kept from task #15, or her own baby branches replacing them? Downstream: energy-branch-content-source resolves part of this.',
  ...
  },
  {
    ...
    effects: 'Node 5 trim (now 3 elements: your hook + documentary + 2 buttons) needs the documentary in slot 2. Two paths: ship Node 5 now with a placeholder and swap when video is ready, OR wait to update Node 5 until the documentary is filmed and ready.',
    suggestion: 'Ship now with a placeholder (a still image, a "documentary coming" card, or the existing intro video) and swap when ready. Lets the new Node 5 architecture land without gating on filming.',
    type: 'binary',
    progress: 25,
    status: 'open',
    options: [
      { label: 'Ship with placeholder', description: 'Update Node 5 to the 3-element layout now; documentary slot uses a placeholder until the real video is ready.', consequence: 'Node 5 ships faster. Two-step delivery (now + later swap).', recommended: true },
      { label: 'Gate Node 5 on video', description: 'Wait to update Node 5 until the documentary is filmed.', consequence: 'Node 5 stays in current state until filming completes. Single-step delivery.' },
    ],
  },
  {
  ...
  // ============ Items resolved or opened 2026-05-16 (architecture pivot) ============
  {
    ...
    effects: 'Node 5 on Sovereign Spiral becomes: (a) Maddie\'s hook paragraph + headline, (b) the documentary, (c) two buttons — "Find out what\'s in your water" (zip dropdown + bottle-water cost breakdown) and "How it may be affecting you / What to do about it →" (deeper dive into GHL pages). Less compositing on Sovereign; water mechanics live in GHL.',
    suggestion: 'Implement the 3-element layout in `src/pages/nodes/[id].astro` for nodeId=5. Documentary slot strategy resolves separately via `documentary-video` (placeholder-then-swap vs gate).',
    type: 'observation',
    progress: 100,
    status: 'resolved',
    notes: 'Layout resolved 2026-05-16 by Maddie (verbatim Node 5 spec in morning iMessage). Implementation work is separate (and depends on `documentary-video` for video-slot strategy) but the layout decision itself is locked.',
  },
  {
    ...
    effects: 'Bottled-water cost breakdown page can be disabled in current state. Maddie wants to preserve the info (don\'t lose it) but defer until GHL migration is straightened out — the breakdown will likely live in GHL with the other water content, not on Sovereign.',
    suggestion: 'Hide the bottled-water cost breakdown from current routes; keep source content + config so it can be re-enabled or migrated to GHL later. Document the freeze.',
    type: 'observation',
    progress: 100,
    status: 'resolved',
  ...
  },
  {
    ...
    ownerNeeded: '4jp',
    urgencyHint: 'Disable on next deploy',
    effects: 'Post-quiz affiliate filter recommendations can be disabled in current state. Maddie wants to preserve the info (don\'t lose it) but defer until GHL migration is straightened out — filter logic will likely live in GHL with tracking + automation.',
    suggestion: 'Hide the post-quiz filter UI; keep source content + tier config so it can be re-enabled or migrated to GHL later. Document the freeze.',
    type: 'observation',
    progress: 100,
    status: 'resolved',
    notes: 'Resolved 2026-05-16 by Maddie: filter content preserved, UI disabled, deferred to post-launch GHL setup. She will send additional filter info/names later for the GHL build.',
  },
  {
    ...
    ownerNeeded: '4jp',
    urgencyHint: 'Ship next',
    effects: 'Maddie chose HTML codes (over GHL-assistant access) as the immediate-delivery path for branches. 6 clean exports already exist at `docs/maddie/2026-05-16-branch-html-exports/*.html` (regenerated after content-leak scrub). Need to deliver to Maddie for paste into GHL.',
    suggestion: 'Send the 6 HTML files via iMessage (screenshot or copy-paste, her preference). Verify against `energy-branch-content-source` first — her GHL table shows 7 slugs, our exports are 6 (no "energy" source mapped).',
    type: 'binary',
    progress: 50,
    status: 'partial',
    options: [
      { label: 'Send 6 now', description: 'Ship the 6 clean exports we have today. Address the missing "energy" branch separately once Maddie confirms its source.', consequence: 'Maddie can start pasting immediately. Energy branch follows when content is resolved.', recommended: true },
      { label: 'Wait for energy resolution', description: 'Hold delivery until all 7 branches are ready including energy.', consequence: 'Single batched delivery; slower start.' },
    ],
    blockedBy: ['energy-branch-content-source'],
  },
  {
    ...
    effects: 'Maddie\'s GHL table shows 7 branches (athletes, fertility, gut-skin, inflammation, energy, planet, cancer). Our Sovereign source has 6 (athletic, fertility, gut-hormones, autoimmune, cancer-support, sustainability). The "energy" slug has no source file in our repo — needs Maddie input on whether it maps to an existing source, lives in her existing GHL content, or needs new copy.',
    suggestion: 'Confirm the 6 mappings (athletes←athletic, fertility←fertility, gut-skin←gut-hormones, inflammation←autoimmune, planet←sustainability, cancer←cancer-support) and tell me where the "energy" branch content lives.',
    type: 'multi',
    progress: 0,
    status: 'open',
    options: [
      { label: 'Maps to athletic', description: 'Athletic branch = the energy branch under a different name.', consequence: 'I rename in the export; 6 branches cover 7 slugs via alias.' },
      { label: 'I have content in GHL', description: 'You\'ll paste your own energy content; I just send 6 branch exports.', consequence: 'No studio action on energy branch; 6 exports ship as-is.' },
      { label: 'Need new copy from studio', description: 'Studio writes a new energy/cellular-health branch from existing source material.', consequence: 'Slower delivery; new branch authored before HTML batch ships.' },
    ],
  },
  {
    ...
    effects: 'Maddie offered to make 4jp her GHL assistant (instead of/in addition to HTML codes delivery). With direct access, studio could wire filter/tracking/automation work without bouncing files back and forth — but adds a security/permissions decision.',
    suggestion: 'Defer until after launch wave stabilizes. HTML codes flow is sufficient for the immediate migration; assistant access pays off when the post-launch filter/tracking work begins.',
    type: 'binary',
    progress: 25,
    status: 'open',
    options: [
      { label: 'Grant after launch', description: 'You add 4jp as a GHL assistant once Sovereign + GHL pages are live.', consequence: 'Studio handles filter/tracking/automation work directly in GHL. Faster iteration on post-launch features.', recommended: true },
      { label: 'Keep HTML-codes-only', description: 'Studio never gets direct GHL access; all changes flow through file delivery.', consequence: 'Slower for any GHL-side iteration; more secure (no third-party access).' },
      { label: 'Grant now', description: 'Add 4jp as GHL assistant immediately to accelerate migration.', consequence: 'Faster migration; one more decision-during-launch instead of after.' },
    ],
  },
];
