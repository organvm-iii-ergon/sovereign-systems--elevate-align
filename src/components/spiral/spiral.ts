...
import {
  ...
  type MathPrimitive,
  type SymmetryType,
} from '../../data/sacred-geometry-primitives';
...
const PHASE_PRESSURE = { solid: 0.1, liquid: 0.3, gas: 0.85 };
...
function generatePhaseNormalMap(
  phase: string,
  seed: number,
): THREE.CanvasTexture {
  const size = 128;
...
function generativeStarGeometry(
  nodeId: number,
  outerR: number,
  depth: number,
): THREE.ExtrudeGeometry {
  ...
  try {
    ...
    nodes.forEach((node, i) => {
      ...
      const accentLeadHex =
        ...
        0xffffff;
      const nodeColor = chakraColorForNode(i, nodes.length).lerp(
        new THREE.Color(accentLeadHex),
        0.32,
      );
      ...
      if (variant === 'stars') {
        ...
        mat = new THREE.MeshPhysicalMaterial({
          ...
          emissiveIntensity: live ? 0.95 : 0.5, // bumped — bloom + saturation = visible color
          metalness: 0.0,
        ...
        mat = new THREE.MeshPhysicalMaterial({
          ...
          emissiveIntensity: live ? 0.6 : 0.3, // dialed back; bloom adds the radiance
      ...
      const avgFieldSize =
        MATERIA_FIELD_SIZE_MAX *
        materia.sizeMul *
        (0.85 + materia.emissiveMul * 0.15);
    ...
    const phaseColors = [
      new THREE.Color(PHASE_HEX.ELEVATE),
      new THREE.Color(PHASE_HEX.ALIGN),
      new THREE.Color(PHASE_HEX.UNLOCK),
    ];

    for (let j = 0; j < AMBIENT_PARTICLE_COUNT; j++) {
      const idx = j * 3;
      ambientBasePositions[idx] =
        (ambientRng() - 0.5) * 2 * AMBIENT_VOLUME_RADIUS;
      ambientBasePositions[idx + 1] =
        (ambientRng() - 0.5) * 2 * AMBIENT_VOLUME_HEIGHT;
      ambientBasePositions[idx + 2] =
        (ambientRng() - 0.5) * 2 * AMBIENT_VOLUME_RADIUS;
  ...
  `;
    ...
    function onPointerUp(e: PointerEvent): void {
      ...
      if (hits.length > 0) {
        ...
        if (data.status === 'live') {
          console.log(
            `[EA] action=spiral_node_click node=${data.id} name=${data.name} phase=${data.phase}`,
          );
    ...
    function loop(): void {
      ...
      orbGroups.forEach((group, i) => {
        ...
        const burstOriginY = Math.cos(node.id * 1.11 + t * 0.13) * 0.1;
        const burstOriginZ = Math.sin(node.id * 1.91 + t * 0.17) * 0.12;
        const burstRadius = 0.24 + world.thermalAmpMul * 0.1;
        ...
        for (let j = 0; j < phaseList.length; j++) {
          ...
          const thermalAmp =
            ...
            (useSpring ? 1.0 : 2.8);
          const pressure =
            PHASE_PRESSURE[part.phase] * (useSpring ? 0.55 : 1.1);
          const gravityScale =
          ...
          for (let s = 0; s < 4; s++) {
            const candidateIdx =
              (j + (s + 1) * (17 + i * 3) + frame * (s + 2)) % phaseList.length;
      ...
      for (let j = 0; j < AMBIENT_PARTICLE_COUNT; j++) {
        ...
        ambientPositions[idx] =
          ambientBasePositions[idx] +
          Math.sin(t * ap.driftFreqX + ap.phase) * ap.driftAmpX;
        ambientPositions[idx + 1] =
          ambientBasePositions[idx + 1] +
          Math.sin(t * ap.driftFreqY + ap.phase * 1.3) * ap.driftAmpY;
        ambientPositions[idx + 2] =
          ambientBasePositions[idx + 2] +
          Math.cos(t * ap.driftFreqZ + ap.phase * 0.7) * ap.driftAmpZ;
      }
